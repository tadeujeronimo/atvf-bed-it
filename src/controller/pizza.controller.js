...
const addCategoriaPizzaController = async (req, res) => {
    try{
        res.status(200).send(await pizzaService.addCategoriaPizzaService(req.params.id, req.body));
    }catch(err){
        console.log(`erro: ${err.message}`);
        return res.status(500).send({ message: `Erro inesperado, tente novamente!`});
    }
...
const removeCategoriaPizzaController = async (req, res) => {
    try{
        res.status(200).send(await pizzaService.removeCategoriaPizzaService(req.params.id, req.body));
    }catch(err){
        console.log(`erro: ${err.message}`);
        return res.status(500).send({ message: `Erro inesperado, tente novamente!`});
    }
