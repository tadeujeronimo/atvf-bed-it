...
module.exports = async (req, res, next) => {
    ...
    if(parts.length !== 2){
        return res.status(401).send({ message: "Token invalido1!"});
    }
    ...
    jwt.verify(token, process.env.SECRET, async (err, decoded) => {
        if(err){
            return res.status(500).send({ message: "Token invalido2!"});
        }

        const user = await findUserByIdService(decoded.id);

        if(!user || !user.id){
            return res.status(401).send({ message: "Token invalido3!"});
        }
