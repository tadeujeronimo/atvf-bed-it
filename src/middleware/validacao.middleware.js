...
const validaPizza = (req, res, next) => {
    ...
    // validar pelo menos um tamanho com preço
    if(!req.body.tamanhos || !Array.isArray(req.body.tamanhos) || req.body.tamanhos.length === 0){
        erros.push("tamanhos");
    }
    if(!req.body.imagem){
        erros.push("imagem");
...
const validaPizzasCarrinhoPedido = (req, res, next) => {
    ...
    // espera um array 'pizzas' com itens { _id, quantidade }
    if(!req.body.pizzas || !Array.isArray(req.body.pizzas) || req.body.pizzas.length === 0){
        return res.status(400).send({ message: `O campo 'pizzas' precisa ser um array com ao menos um item`});
    }
